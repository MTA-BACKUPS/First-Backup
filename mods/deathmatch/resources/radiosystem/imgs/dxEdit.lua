...
local font = dxCreateFont("imgs/VgFont.ttf", 10, false, "proof")
...
function dxDrawEditBox(text, x, y, w, h, password, maxCharacter, element)
	...
	dxDrawRectangle(x, y, w, h, tocolor(45, 45, 45, 0), false)
	if (#getElementData(element, "text2") == 0) then
		dxDrawText(text, x + 5, y, x + w - 10, y + h, tocolor(45, 45, 45, 255), 1, font, "center", "center", true, false, false)
	end
	if (dxGetTextWidth(password and string.gsub(getElementData(element, "text2"), ".", "•") or getElementData(element, "text2"), 1, font) <= w - 10) then
		dxDrawText(password and string.gsub(getElementData(element, "text2"), ".", "•") or getElementData(element, "text2"), x + 5, y, x + w - 10, y + h, tocolor(45, 45, 45, 255), 1, font, "center", "center", true, false, false)
	else
		dxDrawText(password and string.gsub(getElementData(element, "text2"), ".", "•") or getElementData(element, "text2"), x + 5, y, x + w - 10, y + h, tocolor(45, 45, 45, 255), 1, font, "center", "center", true, false, false)
	end
	if (getElementData(element, "state") == true) then
		if (dxGetTextWidth(password and string.gsub(getElementData(element, "text2"), ".", "•") or getElementData(element, "text2"), 1, font) <= w - 10) then
			dxDrawLine(x + 5 + dxGetTextWidth(password and string.gsub(getElementData(element, "text2"), ".", "•") or getElementData(element, "text2"), 1, font), y + 5, x + 5 + dxGetTextWidth(password and string.gsub(getElementData(element, "text2"), ".", "•") or getElementData(element, "text2"), 1, font), y + h - 5, tocolor(45, 45, 45, 0), 1, false)
		else
			dxDrawLine(x + w - 10, y + 5, x + w - 10, y + h - 5, tocolor(45, 45, 45, math.abs(math.sin(getTickCount() / 255) * 255)), 1, false)
		end
	end
...
function dxClickElement(button, state, cx, cy)
	...
	if (button == "left") and (state == "down") then
		...
		for id, element in ipairs(getElementsByType("Vg:RadioFreq")) do
			if (getElementData(element, "mouseState") == "hovered") then
				editBoxId = id
		...
		if (editBoxId) then
			if (isElement(getElementsByType("Vg:RadioFreq")[editBoxId])) then
				if (getElementData(getElementsByType("Vg:RadioFreq")[editBoxId], "state") == false) then
					guiSetInputMode("no_binds")
					setElementData(getElementsByType("Vg:RadioFreq")[editBoxId], "state", true)
				end
			end
		end
	end
...
function dxCharacterElement(button)
	...
	for _, element in ipairs(getElementsByType("Vg:RadioFreq")) do
		if (getElementData(element, "state") == true) then
